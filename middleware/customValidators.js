...
async function checkItemNumber(value) {
    if (value === "") {
        throw new ItemAddError("item number required");
    }
    ...
    if (!itemNumber) {
        throw new ItemAddError("Item number not valid number");
    }

    const itemNumberExists =
        await itemQueries.itemQueryExactNumber(itemNumberString);
    if (itemNumberExists) {
        throw new ItemAddError("Item number already exists");
    }
...
async function checkItemBrand(value) {
    if (value === "") {
        throw new ItemAddError("Item brand required");
    }
...
async function checkItemWeight(value) {
    if (value === "") {
        throw new ItemAddError("Item weight required");
    }
    let itemWeight = Number(value);
    if (!itemWeight || itemWeight <= 0) {
        throw new ItemAddError("Invalid value for item weight");
    }
...
async function checkItemPallet(value) {
    if (value === "") {
        throw new ItemAddError("Full Pallet Quantity required");
    }
    let itemPallet = Number(value);
    if (!itemPallet || itemPallet <= 0) {
        throw new ItemAddError("Invalid value for full pallet quantity");
    }
...
async function checkItemLocations(value) {
    ...
    if (locations === "") {
        throw new ItemAddError("Item location required");
    }
    ...
    for (const loc of locationList) {
        ...
        if (!locId) {
            throw new ItemAddError(`Location '${loc}' does not exist`);
        }
...
async function testValidateNewItem(req, res, next) {
    const newItem = {
...
export default testValidateNewItem;
